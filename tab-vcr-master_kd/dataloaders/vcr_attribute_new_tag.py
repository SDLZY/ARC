...
class VCR(Dataset):
    def __init__(self, split, mode, only_use_relevant_dets=True, add_image_as_a_box=True, embs_to_load='bert_da',
        ...
        self.h5fn = os.path.join(VCR_ANNOTS_DIR, split, f'{self.embs_to_load}_{self.mode}_{self.split}.h5')
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'answer':
            ...
            for k,v in non_tag_answer_annotid2detidx[conditioned_label].items():           # 这里是个bug，因为有两个连续object的情况下是一个list，在计算长度的时候只能算一个，是错误的，如果消融实验还可以，那这里就不改了（这里没有错误，对应上面的_my_fix_tokenization来看）
