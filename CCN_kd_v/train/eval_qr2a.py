...
from dataloaders.vcr_qr2a import VCR, VCRLoader
...
parser = argparse.ArgumentParser(description='eval_qr2a')
parser.add_argument(
    '-folder',
    type=str,
)
...
folder = args.folder
params = Params.from_file(os.path.join(folder, 'qr2a.json'))
train, val, test = VCR.splits(mode='answer',embs_to_load='bert_qr2a', only_use_relevant_dets=True)
NUM_THREADS = 12
...
model.load_state_dict(torch.load(os.path.join(folder, 'best.th')))
...
print("Final train accuracy is {:.3f}".format(acc))
np.save(os.path.join(folder, f'train_logits.npy'), train_logits)
np.save(os.path.join(folder, f'train_preds.npy'), train_probs)
...
np.save(os.path.join(folder, f'val_logits.npy'), val_logits)
np.save(os.path.join(folder, f'val_preds.npy'), val_probs)
