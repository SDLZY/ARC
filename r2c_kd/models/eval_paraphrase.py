...
from dataloaders.vcr_kd_paraphrase import VCR, VCRLoader, get_batch_tasks2
...
args.params = '/home/share/lizhenyang/r2c_kd_lzy_二轮时的内容/models/kd/model_kd_infonce.json'                                      #############
args.folder = '/home/share/lizhenyang/r2c_kd_lzy_二轮时的内容/models/kd/saves/Q2A-T20-alpha01_QA2R-T1-beta01_统一优化器'                                      #############
# args.plot = '/home/share/lizhenyang/r2c_kd_lzy/models/kd/saves/Q2A-T15-alpha01_QA2R-T1-beta01_统一优化器/runs'
alpha = 0.1
beta = 0.1
...
#     shutil.copy2('/home/share/lizhenyang/r2c_kd_lzy/models/train_kd_infonce.py', args.folder)
logger = open('/home/share/lizhenyang/r2c_kd_lzy_二轮时的内容/models/kd/saves/Q2A-T20-alpha01_QA2R-T1-beta01_统一优化器/log_paraphrase-retrain64.txt', mode='a', encoding='utf8')    #############

TASKS = '1', '2'
val = VCR.splits(tasks=TASKS, embs_to_load='bert_da', logits_qr2a=True, features_penult=True,
...
model = Model.from_params(vocab=val.vocab, params=params['model'])
...
for epoch_num in range(0, 1):
    ...
    #     batch = get_batch_tasks2(batch, TASKS)
    #     optimizer.zero_grad()
    ...
    #     nce_t = output_dict['nce_t'].mean()
    #     nce_s = output_dict['nce_s'].mean()
    #     ncep_t = output_dict['ncep_t'].mean()
    #     ncep_s = output_dict['ncep_s'].mean()
    #     nceacc_t = output_dict['nceacc_t'].mean()
    #     nceacc_s = output_dict['nceacc_s'].mean()
    #     loss = loss_kd_1 * alpha + loss_1 * (1 - alpha) + loss_2 * (1 - beta) + loss_infonce * beta + output_dict['cnn_regularization_loss'].mean()
    #
    #     # losses = {}
    #     # for task in TASKS:
    #     #     losses[f'loss_{task}'] = output_dict[f'loss_{task}'].mean()
    #     # losses['crl'] = output_dict['cnn_regularization_loss'].mean()
    #     # if 'loss_kd' in output_dict:
    #     #     losses['loss_kd'] = output_dict['loss_kd'].mean()
    ...
    #         'ncep_t': ncep_t.item(),
    #         'ncep_s': ncep_s.item(),
    #         'nceacc_t': nceacc_t.item(),
    #         'nceacc_s': nceacc_s.item(),
    ...
    # writer.add_scalar('loss_kd_1/train', epoch_results['loss_kd_1'], epoch_num)
    ...
    epoch_stats.to_csv(os.path.join(args.folder, f'val_stats_epoch_{epoch_num}_paraphrase.csv'))
    hits = {}
    val_acc = {}
    for task in TASKS:
    ...
    #     "accuracy_qa2r_c": val_qa2r_c_acc,
    #     "accuracy_qa2r_w": val_qa2r_w_acc,
    ...
    print(f"Val epoch {epoch_num}  q2a_acc {val_acc['1']:.4f} qa2r_acc {val_acc['2']:.4f} "
          f"(c {val_qa2r_c_acc:.4f} w {val_qa2r_w_acc:.4f}) q2ar_acc {val_q2ar_acc:.4f} ",
          flush=True)
    ...
    # writer.add_scalar('accuracy_qa2r_c/validation', val_metric_per_epoch[-1]['accuracy_qa2r_c'], epoch_num)
    # writer.add_scalar('accuracy_qa2r_w/validation', val_metric_per_epoch[-1]['accuracy_qa2r_w'], epoch_num)
...
logger.write(f"Final val q2a accuracy is {val_acc['1']:.4f} qa2r accuracy is {val_acc['2']:.4f} (c {val_qa2r_c_acc:.4f} "
      f"w {val_qa2r_w_acc:.4f}) q2ar accuracy is {val_q2ar_acc:.4f}")
