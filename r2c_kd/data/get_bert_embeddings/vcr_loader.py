...
def process_ctx_ans_for_bert(ctx_raw, ans_raw, tokenizer, counter, endingonly, max_seq_length, is_correct):
    ...
    if endingonly:
        # sequence中只有一个sentence的情况
        take_away_from_ctx = len(answer[0]) - max_seq_length + 2  # [CLS] ... [SEP]
    ...
    len_total = len(context[0]) + len(answer[0]) + 3  # [CLS] ... [SEP] ... [SEP]
