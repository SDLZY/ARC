...
def kd_loss(logits_t, logits_s, T):
    logp_s = F.log_softmax(logits_s / T, dim=1)
    p_t = F.softmax(logits_t / T, dim=1)
    loss = F.kl_div(logp_s, p_t, reduction='batchmean') * (T ** 2)
    return loss
...
def save_checkpoint(model, optimizer, serialization_dir, epoch, val_metric_per_epoch, is_best=None,
    ...
    if serialization_dir is not None:
        ...
        if isinstance(optimizer, dict):
            training_state['optimizer'] = dict()
            for key, value in optimizer.items():
                training_state['optimizer'][key] = value.state_dict()
        else:
            training_state['optimizer'] = optimizer.state_dict()
        if learning_rate_scheduler is not None:
            if isinstance(learning_rate_scheduler, dict):
                training_state["learning_rate_scheduler"] = dict()
                for key, value in learning_rate_scheduler.items():
                    training_state['learning_rate_scheduler'][key] = value.lr_scheduler.state_dict()
            else:
...
def restore_checkpoint(model, optimizer, serialization_dir, learning_rate_scheduler=None):
    ...
    if isinstance(optimizer, dict):
        for key, value in optimizer.items():
            value.load_state_dict(training_state['optimizer'][key])
    else:
    ...
    if learning_rate_scheduler is not None and "learning_rate_scheduler" in training_state:
        if isinstance(learning_rate_scheduler, dict):
            for key, value in learning_rate_scheduler.items():
                value.lr_scheduler.load_state_dict(training_state['learning_rate_scheduler'][key])
        else:
